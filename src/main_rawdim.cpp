...
int main() {
    try {
        // SIFT
        Dataset base = load_data_hdf5("../data/sift-128-euclidean.hdf5", "/train");
        Dataset queries = load_data_hdf5("../data/sift-128-euclidean.hdf5", "/test");
        Dataset neighbors = load_data_hdf5("../data/sift-128-euclidean.hdf5", "/neighbors");
        ...
        start = std::chrono::high_resolution_clock::now();
        // 测试插入操作
        Vector new_vec(128, 0.1);
        index.insert(new_vec);
        // 测试删除操作
        auto prev_results = index.search(queries[dis(gen)], k);
